...
import {
    setNewLimit, setWarmLimit, setTrustedLimit, softBan, reconnect,
    getProtection,
} from "@/features/admin/services/admin-service";
import { Shield, Loader2, Ban, RefreshCw, Users, Flame, Star } from "lucide-react";
...
export default function AdminProtectionPage() {
    ...
    const [trustedLimit, setTrustedLimitState] = useState({ perMin: 60, perHour: 600 });
    ...
    // Reconnect
    const [reconnectSettings, setReconnectSettings] = useState({ baseMs: 500, maxMs: 5000 });
    ...
    const handleSetLimit = async (
        ...
        try {
            ...
            toast.success(`${type.charAt(0).toUpperCase() + type.slice(1)} limits updated`);
        } catch (err: unknown) {
            toast.error((err as { message?: string })?.message ?? "Failed to update limits");
        } finally {
            setLoading(null);
        }
    };
    ...
    const handleSoftBan = async () => {
        ...
        try {
            ...
            toast.error((err as { message?: string })?.message ?? "Failed to apply soft ban");
        } finally {
            setLoading(null);
        }
    };
    ...
    const handleReconnect = async () => {
        ...
        try {
            ...
            toast.error((err as { message?: string })?.message ?? "Failed to update reconnect settings");
        } finally {
            setLoading(null);
        }
    };
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Card className="border border-border">
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-1.5">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setNewLimitState((p) => ({ ...p, perMin: Number(e.target.value) }))}
                                    min={1}
                                />
                            </div>
                            <div className="space-y-1.5">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setNewLimitState((p) => ({ ...p, perHour: Number(e.target.value) }))}
                                    min={1}
                                />
                            </div>
                        </div>
                        <Button
                            ...
                            onClick={() => handleSetLimit("new", newLimit)}
                            disabled={loading === "new"}
                        >
                            {loading === "new" ? <Loader2 className="h-4 w-4 animate-spin me-2" /> : null}
                            Apply
                        </Button>
                    </CardContent>
                </Card>
                ...
                <Card className="border border-border">
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-1.5">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setWarmLimitState((p) => ({ ...p, perMin: Number(e.target.value) }))}
                                    min={1}
                                />
                            </div>
                            <div className="space-y-1.5">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setWarmLimitState((p) => ({ ...p, perHour: Number(e.target.value) }))}
                                    min={1}
                                />
                            </div>
                        </div>
                        <Button
                            className="w-full" size="sm"
                            onClick={() => handleSetLimit("warm", warmLimit)}
                            disabled={loading === "warm"}
                        >
                            {loading === "warm" ? <Loader2 className="h-4 w-4 animate-spin me-2" /> : null}
                            Apply
                        </Button>
                    </CardContent>
                </Card>
                ...
                <Card className="border border-border">
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-1.5">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setTrustedLimitState((p) => ({ ...p, perMin: Number(e.target.value) }))}
                                    min={1}
                                />
                            </div>
                            <div className="space-y-1.5">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setTrustedLimitState((p) => ({ ...p, perHour: Number(e.target.value) }))}
                                    min={1}
                                />
                            </div>
                        </div>
                        <Button
                            className="w-full" size="sm"
                            onClick={() => handleSetLimit("trusted", trustedLimit)}
                            disabled={loading === "trusted"}
                        >
                            {loading === "trusted" ? <Loader2 className="h-4 w-4 animate-spin me-2" /> : null}
                            Apply
                        </Button>
                    </CardContent>
                </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Card className="border border-border">
                    ...
                    <CardContent className="space-y-4">
                        ...
                        >
                            {loading === "ban" ? <Loader2 className="h-4 w-4 animate-spin me-2" /> : <Ban className="h-4 w-4 me-2" />}
                            Apply Soft Ban
                        </Button>
                    </CardContent>
                </Card>
                ...
                <Card className="border border-border">
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-1.5">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setReconnectSettings((p) => ({ ...p, baseMs: Number(e.target.value) }))}
                                    min={100}
                                    placeholder="500"
                                />
                            </div>
                            <div className="space-y-1.5">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setReconnectSettings((p) => ({ ...p, maxMs: Number(e.target.value) }))}
                                    min={100}
                                    placeholder="5000"
                                />
                            </div>
                        ...
                        <p className="text-xs text-muted-foreground">
                            Reconnect will retry between {reconnectSettings.baseMs}ms and {reconnectSettings.maxMs}ms
                        </p>
                        <Button
                        ...
                        >
                            {loading === "reconnect" ? <Loader2 className="h-4 w-4 animate-spin me-2" /> : <RefreshCw className="h-4 w-4 me-2" />}
                            Update Settings
                        </Button>
                    </CardContent>
                </Card>
